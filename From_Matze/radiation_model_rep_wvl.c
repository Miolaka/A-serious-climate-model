...
void sort_vertically(double Theta[], int nlay) {
    ...
    while (unstable) {
        unstable = 0;
        for (int i = nlay - 2; i >= 0; i--) { // 9,8,7,6,5,...,0
...
void get_Theta_from_T(double Theta[], double T[]) {
...
void get_T_from_Theta(double Theta[], double T[]) {
    for (int i = nlay - 1; i >= 0; i--) {
...
// only used for tau_LW_abs_mol so far

inline double calc_epsilon(double tau_LW_abs_mol, double mu) {
    return 1.0 - exp(- tau_LW_abs_mol / mu);
}
...
//Calculate B plancksche Strahlung:
    ...
    for(int i=nlev-2; i>=0; i--){
        B[i] = weight[w]*calc_B(T[i], lambda);
     }
