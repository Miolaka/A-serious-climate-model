...
int main(int argc, char **argv)
{
    ...
    double p_delta = p_ground/nlevel; //100hPa per level //Paul: check: divide by nlevel or nlayer?
    double p_middle;
    int ntime = 20000;
    //double heat_temp = 1;
    //legacy:
    ...
    int delta_time = 60*60*1*1; //1 stunden //Paul: Better go for double here
    double delta_Temp[nlevel]; //Paul: Nlevel or Nlayer? ;)
    ...
    double lambda_end = 26.0*pow(10.0,-6.0); //Paul: You might want to go a little further than 26 mu
    int gridnumber = 20;
